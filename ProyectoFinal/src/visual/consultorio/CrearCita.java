...
public class CrearCita extends JDialog {
        ...
        public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            ...
            if (value instanceof DoctorItem) {
                ...
                if (item.getDoctor() == null) {
                    ...
                    String especialidad = (d.getEspecialidad() != null && d.getEspecialidad().getNombre() != null)
    ...
    public CrearCita(Cita cita) {
        ...
        if (citaEditar != null) {
            ...
            lblCodigo.setBounds(20, 30, 55, 22);
            panel.add(lblCodigo);

            txtIdCita = new JTextField(citaEditar.getId());
            ...
            txtIdCita.setBounds(75, 30, 95, 22);
            panel.add(txtIdCita);

            JLabel lblFecha = new JLabel("Fecha:");
            lblFecha.setForeground(new Color(70, 130, 180));
            lblFecha.setFont(new Font("Dialog", Font.BOLD, 12));
            lblFecha.setBounds(180, 30, 45, 22);
            panel.add(lblFecha);

            Date fechaInicial = (citaEditar.getFechaConsulta() != null) ? citaEditar.getFechaConsulta() : new Date();
            ...
            spnFecha.setBounds(225, 30, 100, 22);
            panel.add(spnFecha);

            JLabel lblHora = new JLabel("Hora:");
            lblHora.setForeground(new Color(70, 130, 180));
            lblHora.setFont(new Font("Dialog", Font.BOLD, 12));
            lblHora.setBounds(335, 30, 40, 22);
            panel.add(lblHora);

            Date horaInicial = (citaEditar.getHoraConsulta() != null) ? citaEditar.getHoraConsulta() : new Date();
            spnHora = new JSpinner(new SpinnerDateModel(horaInicial, null, null, Calendar.HOUR_OF_DAY));
            spnHora.setFont(new Font("Dialog", Font.PLAIN, 12));
            spnHora.setBorder(new LineBorder(new Color(70, 130, 180)));
            Formato.setSpinner(spnHora);
            Formato.colorSpinner(spnHora, new Color(240, 248, 255));
            ...
            lblFecha.setForeground(new Color(70, 130, 180));
            lblFecha.setFont(new Font("Dialog", Font.BOLD, 12));
            lblFecha.setBounds(65, 30, 45, 22);
            panel.add(lblFecha);
            ...
            Formato.setSpinner(spnFecha);
            Formato.colorSpinner(spnFecha, new Color(240, 248, 255));
            spnFecha.setBounds(112, 30, 135, 22);
            panel.add(spnFecha);

            JLabel lblHora = new JLabel("Hora:");
            lblHora.setForeground(new Color(70, 130, 180));
            lblHora.setFont(new Font("Dialog", Font.BOLD, 12));
            lblHora.setBounds(267, 30, 40, 22);
            panel.add(lblHora);
            ...
            Formato.colorSpinner(spnHora, new Color(240, 248, 255));
            spnHora.setEditor(new JSpinner.DateEditor(spnHora, "hh:mm a"));
            spnHora.setBounds(309, 30, 135, 22);
            panel.add(spnHora);
        }
    ...
    private void registrarCita() {
        ...
        Date horaSel = (Date) spnHora.getValue();
        Time horaConsulta = new Time(horaSel.getTime());
        ...
        if (citaEditar == null) {
            ...
            CitaDAO.getInstance().guardarCita(nuevaCita);
    ...
    private void cargarDatosModificacion() {
        ...
        if (citaEditar.getFechaConsulta() != null) {
            spnFecha.setValue(citaEditar.getFechaConsulta());
        }
        if (citaEditar.getHoraConsulta() != null) {
            spnHora.setValue(citaEditar.getHoraConsulta());
        }
        ...
        if (citaEditar.getDoctor() != null) {
            ...
            for (int i = 0; i < cbxDoctor.getItemCount(); i++) {
                ...
                if (item != null && item.getDoctor() != null && item.getDoctor().getId().equalsIgnoreCase(auxDoctor.getId())) {
                    ...
                    String esp = (auxDoctor.getEspecialidad() != null && auxDoctor.getEspecialidad().getNombre() != null)
                            ? auxDoctor.getEspecialidad().getNombre() : "General";
    ...
    private void abrirVentanaCrearPaciente() {
        ...
        CrearPaciente regPacienteDialog = new CrearPaciente(null);
    ...
    private List<Doctor> obtenerDoctoresFiltrados(String filtro) {
        ...
        if (Clinica.getInstancia().getDoctores() != null) {
            for (Doctor d : Clinica.getInstancia().getDoctores()) {
                ...
                boolean matchEspecialidad = d.getEspecialidad() != null &&
                        d.getEspecialidad().getNombre() != null &&
                        d.getEspecialidad().getNombre().toLowerCase().contains(filtroNorm);
    ...
    private void seleccionarDoctor() {
        ...
        if (itemSeleccionado instanceof DoctorItem) {
            ...
            if (item.getDoctor() != null) {
                ...
                String esp = (doc.getEspecialidad() != null && doc.getEspecialidad().getNombre() != null)
                        ? doc.getEspecialidad().getNombre() : "General";
    ...
    private void limpiarCampos() {
        ...
        lblInfoDoctor.setForeground(new Color(120, 130, 140));
        if (txtIdCita != null) {
            txtIdCita.setText(citaEditar != null ? citaEditar.getId() : "");
        }
    }
}